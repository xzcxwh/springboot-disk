...
export default {
  ...
  methods: {
    // 保存分享的方法
    saveShare() {
      // 如果 shareLink 已经有值（即已经分享过），则隐藏分享操作界面
      if (this.shareLink) {
        this.shareVisible = false
        return // 提前返回，不再执行后续的分享操作
      }
      // 使用 post 方法向 '/diskFiles/share/' 发送请求，携带表单数据 this.form
      this.$request.post('/diskFiles/share/', this.form).then(res => {
        // 如果请求返回的响应码是 '200'，表示操作成功
        if (res.code === '200') {
          // 显示操作成功的提示信息
